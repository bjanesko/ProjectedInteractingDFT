...
BGJ modified to treat restrained denominator RDMP2 JCTC 2014, 10, 4857 Ten-no and
coworkers 
'''
...
class DFURDMP2(DFRMP2):
    '''
    native implementation of DF-MP2/RI-MP2 with a UHF reference
    '''

    def __init__(self, mf, frozen=None, auxbasis=None, tau=2.39):
        '''
        Args:
            mf : UHF instance
            ...
            BGJ tau: minimum denominator for restraint, default to 65 eV
        '''
    ...
    def calculate_energy(self):
        '''
        Calculates the RDMP2 correlation energy.
        '''
        ...
        logger = lib.logger.new_logger(self)
        logger.info('')
        logger.info('Calculating DF-RDMP2 energy')
        self.e_corr = emp2_uhf(self._intsfile, self.mo_energy, self.tau, self.frozen_mask,
                               logger, ps=self.ps, pt=self.pt)
        logger.note('DF-RDMP2 correlation energy: {0:.14f}'.format(self.e_corr))
        return self.e_corr
    ...
    def calculate_integrals_(self):
        ...
        print('LOOK WE ARE CALCULATING INTEGRALS WITH MAX MEMORY ',self.max_memory)
        for s in [0, 1]:
...
RDMP2 = URDMP2 = DFRDMP2 = DFURDMP2


class SCSDFURDMP2(DFURDMP2):
    '''
    UHF-DF-RD-MP2 with spin-component scaling
...
SCSRDMP2 = SCSURDMP2 = SCSDFRDMP2 = SCSDFURDMP2


def emp2_uhf(intsfiles, mo_energy, tau, frozen_mask, logger, ps=1.0, pt=1.0):
    '''
    Calculates the DF-RDMP2 energy with an UHF reference.
    ...
    logger.debug('    UHF-DF-RDMP2 energy routine')
    ...
    logger.debug('    DF-RDMP2 correlation energy: {0:.14f}'.format(energy_total))
    return energy_total
